# 服务器代理架构

<cite>
**本文引用的文件**   
- [ServerAPIClient.swift](file://Services/ServerAPIClient.swift)
- [AISummaryService.swift](file://Services/AISummaryService.swift)
- [CosyVoiceService.swift](file://Services/CosyVoiceService.swift)
- [CompanionService.swift](file://Services/CompanionService.swift)
- [EdgeTTSService.swift](file://Services/EdgeTTSService.swift)
- [EdgeTTSSynthesizer.swift](file://Services/EdgeTTSSynthesizer.swift)
- [SpeechService.swift](file://Services/SpeechService.swift)
- [AudioSessionService.swift](file://Services/AudioSessionService.swift)
- [SummaryResult.swift](file://Models/SummaryResult.swift)
- [VoiceConfig.swift](file://Models/VoiceConfig.swift)
- [PlaybackState.swift](file://Models/PlaybackState.swift)
- [ContentView.swift](file://Views/ContentView.swift)
- [AppDelegate.swift](file://App/AppDelegate.swift)
- [KnowledgeApp.swift](file://App/KnowledgeApp.swift)
- [Info.plist](file://Resources/Info.plist)
</cite>

## 更新摘要
**所做更改**   
- 更新了服务器代理客户端配置，指向生产环境 DeepSeek AI 代理服务
- 增强了响应格式兼容性，支持多种 API 返回格式
- 新增了 Edge TTS 云端语音合成服务
- 完善了架构图表以反映新的服务组件

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与可靠性](#性能与可靠性)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本仓库为 iOS 应用"挠荔枝"，其核心目标是提供文档阅读、AI 摘要、伴读对话以及语音合成（TTS）能力。为保障 API Key 安全并统一接入第三方 AI 服务，客户端采用"服务器代理"模式：所有 AI 请求均通过自建中转服务器转发，避免在客户端暴露敏感凭据。本文聚焦于该代理架构的设计、数据流、错误处理与扩展点，帮助读者快速理解并在此基础上进行二次开发或部署。

## 项目结构
整体采用分层组织：
- App 层：应用入口与全局配置
- Services 层：业务服务与网络代理
- Models 层：领域模型与配置
- Views 层：UI 展示与交互
- Resources 层：应用资源与权限声明

```mermaid
graph TB
subgraph "应用入口"
A["AppDelegate.swift"]
B["KnowledgeApp.swift"]
P["Info.plist"]
end
subgraph "视图层"
V1["ContentView.swift"]
end
subgraph "服务层"
S1["AISummaryService.swift"]
S2["CompanionService.swift"]
S3["CosyVoiceService.swift"]
S4["EdgeTTSService.swift"]
S5["EdgeTTSSynthesizer.swift"]
S6["SpeechService.swift"]
S7["AudioSessionService.swift"]
C["ServerAPIClient.swift"]
end
subgraph "模型层"
M1["SummaryResult.swift"]
M2["VoiceConfig.swift"]
M3["PlaybackState.swift"]
end
A --> S7
B --> V1
V1 --> S1
V1 --> S2
V1 --> S3
V1 --> S5
S1 --> C
S2 --> C
S3 --> C
S4 --> S5
S5 --> S4
S6 --> S7
S6 --> M3
S6 --> M2
S1 --> M1
P --> A
```

图表来源
- [AppDelegate.swift:1-14](file://App/AppDelegate.swift#L1-L14)
- [KnowledgeApp.swift:1-29](file://App/KnowledgeApp.swift#L1-L29)
- [Info.plist:1-44](file://Resources/Info.plist#L1-L44)
- [ContentView.swift:1-98](file://Views/ContentView.swift#L1-L98)
- [AISummaryService.swift:1-90](file://Services/AISummaryService.swift#L1-L90)
- [CompanionService.swift:1-47](file://Services/CompanionService.swift#L1-L47)
- [CosyVoiceService.swift:1-104](file://Services/CosyVoiceService.swift#L1-L104)
- [EdgeTTSService.swift:1-133](file://Services/EdgeTTSService.swift#L1-L133)
- [EdgeTTSSynthesizer.swift:1-275](file://Services/EdgeTTSSynthesizer.swift#L1-L275)
- [SpeechService.swift:1-166](file://Services/SpeechService.swift#L1-L166)
- [AudioSessionService.swift:1-46](file://Services/AudioSessionService.swift#L1-L46)
- [SummaryResult.swift:1-33](file://Models/SummaryResult.swift#L1-L33)
- [VoiceConfig.swift:1-71](file://Models/VoiceConfig.swift#L1-L71)
- [PlaybackState.swift:1-9](file://Models/PlaybackState.swift#L1-L9)

章节来源
- [AppDelegate.swift:1-14](file://App/AppDelegate.swift#L1-L14)
- [KnowledgeApp.swift:1-29](file://App/KnowledgeApp.swift#L1-L29)
- [Info.plist:1-44](file://Resources/Info.plist#L1-L44)
- [ContentView.swift:1-98](file://Views/ContentView.swift#L1-L98)

## 核心组件
- 服务器代理客户端：集中管理 HTTP 请求构建、超时、响应校验与错误映射，屏蔽下游差异，向上提供服务化接口。
- 业务服务：封装具体功能（摘要、伴读、TTS），复用代理客户端，负责参数组装、结果解析与状态维护。
- Edge TTS 服务：通过 naolizhi.cn 中转调用微软 Edge TTS，提供免费无限量的高质量中文语音合成。
- 播放服务：基于系统 TTS 的本地播放控制，支持分段朗读、进度回调与状态机。
- 音频会话服务：统一管理 AVAudioSession 的配置、激活与停用，确保后台播放与蓝牙/AirPlay 兼容。
- 模型与配置：定义摘要结果、语音配置与播放状态等数据结构。

章节来源
- [ServerAPIClient.swift:1-206](file://Services/ServerAPIClient.swift#L1-L206)
- [AISummaryService.swift:1-90](file://Services/AISummaryService.swift#L1-L90)
- [CompanionService.swift:1-47](file://Services/CompanionService.swift#L1-L47)
- [CosyVoiceService.swift:1-104](file://Services/CosyVoiceService.swift#L1-L104)
- [EdgeTTSService.swift:1-133](file://Services/EdgeTTSService.swift#L1-L133)
- [EdgeTTSSynthesizer.swift:1-275](file://Services/EdgeTTSSynthesizer.swift#L1-L275)
- [SpeechService.swift:1-166](file://Services/SpeechService.swift#L1-L166)
- [AudioSessionService.swift:1-46](file://Services/AudioSessionService.swift#L1-L46)
- [SummaryResult.swift:1-33](file://Models/SummaryResult.swift#L1-L33)
- [VoiceConfig.swift:1-71](file://Models/VoiceConfig.swift#L1-L71)
- [PlaybackState.swift:1-9](file://Models/PlaybackState.swift#L1-L9)

## 架构总览
下图展示了从 UI 到服务端代理的关键调用链与数据流向。

```mermaid
sequenceDiagram
participant UI as "ContentView"
participant SumSvc as "AISummaryService"
participant CompSvc as "CompanionService"
participant VoiceSvc as "CosyVoiceService"
participant EdgeTTS as "EdgeTTSService"
participant Client as "ServerAPIClient"
participant Server as "naolizhi.cn 中转服务器"
UI->>SumSvc : "生成摘要(文本)"
SumSvc->>Client : "POST /summary"
Client->>Server : "HTTP POST JSON"
Server-->>Client : "JSON {result|content}"
Client-->>SumSvc : "字符串内容"
SumSvc-->>UI : "SummaryResult"
UI->>CompSvc : "提问(问题,上下文,历史)"
CompSvc->>Client : "POST /companion"
Client->>Server : "HTTP POST JSON"
Server-->>Client : "JSON {result|content}"
Client-->>CompSvc : "字符串回复"
CompSvc-->>UI : "AI 回复"
UI->>VoiceSvc : "TTS(文本,音色ID,语速)"
VoiceSvc->>Client : "POST /tts"
Client->>Server : "HTTP POST JSON"
Server-->>Client : "音频二进制或含URL/Base64的JSON"
Client-->>VoiceSvc : "Data(音频)"
VoiceSvc-->>UI : "音频数据"
UI->>EdgeTTS : "Edge TTS(文本,音色,语速)"
EdgeTTS->>Server : "POST /api/tts"
Server-->>EdgeTTS : "MP3 音频数据"
EdgeTTS-->>UI : "音频数据"
```

图表来源
- [ContentView.swift:1-98](file://Views/ContentView.swift#L1-L98)
- [AISummaryService.swift:1-90](file://Services/AISummaryService.swift#L1-L90)
- [CompanionService.swift:1-47](file://Services/CompanionService.swift#L1-L47)
- [CosyVoiceService.swift:1-104](file://Services/CosyVoiceService.swift#L1-L104)
- [EdgeTTSService.swift:1-133](file://Services/EdgeTTSService.swift#L1-L133)
- [ServerAPIClient.swift:1-206](file://Services/ServerAPIClient.swift#L1-L206)

## 详细组件分析

### 服务器代理客户端（ServerAPIClient）
职责
- 统一构建请求、设置超时、校验响应码与类型
- 抽象通用 JSON 提取逻辑（兼容多格式）
- 提供总结、伴读、TTS、语音克隆等接口
- 将网络与协议细节封装为结构化错误

**更新** 已配置为生产环境 DeepSeek AI 代理服务，支持多种响应格式兼容性

关键流程
- 请求构建：拼接 baseURL + endpoint，设置 Content-Type 与 JSON Body
- 响应校验：按状态码分类抛出语义化错误
- 内容提取：优先取 result；兼容 content；兼容 output.choices[*].message.content
- TTS 特殊处理：直接返回 audio 二进制，或解析 JSON 中的 audio_url/audio base64

```mermaid
flowchart TD
Start(["进入 post(endpoint,body)"]) --> Build["构建 URLRequest<br/>baseURL: https://naolizhi.cn/api"]
Build --> Send["发送请求并等待响应"]
Send --> CheckResp{"是否为 HTTP 响应?"}
CheckResp -- 否 --> ErrInvalid["抛出无效响应错误"]
CheckResp -- 是 --> Validate["校验状态码"]
Validate --> CodeOK{"状态码 200?"}
CodeOK -- 否 --> MapErr["映射为未授权/配额不足/服务器错误"]
CodeOK -- 是 --> ParseJSON["解析 JSON"]
ParseJSON --> ExtractResult{"是否包含 result?"}
ExtractResult -- 是 --> ReturnStr["返回 result 字符串"]
ExtractResult -- 否 --> CheckContent{"是否包含 content?"}
CheckContent -- 是 --> ReturnStr
CheckContent -- 否 --> Fallback{"兼容 output.choices.message.content?"}
Fallback -- 是 --> ReturnStr
Fallback -- 否 --> ErrInvalid
```

图表来源
- [ServerAPIClient.swift:101-173](file://Services/ServerAPIClient.swift#L101-L173)

章节来源
- [ServerAPIClient.swift:1-206](file://Services/ServerAPIClient.swift#L1-L206)

### AI 摘要服务（AISummaryService）
职责
- 调用代理客户端获取原始文本
- 解析结构化摘要（正文与要点）
- 输出 SummaryResult

```mermaid
classDiagram
class AISummaryService {
+generateSummary(for : text,maxLength : ) SummaryResult
-parseSummaryResponse(response : String) SummaryResult
}
class SummaryResult {
+string content
+string[] keyPoints
+date createdAt
}
AISummaryService --> SummaryResult : "构造并返回"
```

图表来源
- [AISummaryService.swift:1-90](file://Services/AISummaryService.swift#L1-L90)
- [SummaryResult.swift:1-33](file://Models/SummaryResult.swift#L1-L33)

章节来源
- [AISummaryService.swift:1-90](file://Services/AISummaryService.swift#L1-L90)
- [SummaryResult.swift:1-33](file://Models/SummaryResult.swift#L1-L33)

### AI 伴读服务（CompanionService）
职责
- 维护多轮对话历史（最多 20 条消息）
- 携带当前朗读上下文向服务器发起对话请求
- 返回 AI 回复文本

```mermaid
sequenceDiagram
participant UI as "调用方"
participant Comp as "CompanionService"
participant Client as "ServerAPIClient"
participant Server as "naolizhi.cn 中转服务器"
UI->>Comp : "ask(question,context,maxTokens)"
Comp->>Client : "POST /companion (question,context,history)"
Client->>Server : "HTTP POST JSON"
Server-->>Client : "JSON {result|content}"
Client-->>Comp : "字符串回复"
Comp->>Comp : "追加用户与助手消息至 history"
Comp-->>UI : "AI 回复"
```

图表来源
- [CompanionService.swift:1-47](file://Services/CompanionService.swift#L1-L47)
- [ServerAPIClient.swift:35-45](file://Services/ServerAPIClient.swift#L35-L45)

章节来源
- [CompanionService.swift:1-47](file://Services/CompanionService.swift#L1-L47)

### CosyVoice 语音合成服务（CosyVoiceService）
职责
- 封装 TTS 与语音克隆接口
- 提供分段合成与进度回调
- 透传音频数据给上层播放器

```mermaid
classDiagram
class CosyVoiceService {
+synthesize(text,voiceId,rate) Data
+cloneVoice(audioData,voiceName) String
+previewVoice(voiceId,sampleText) Data
+synthesizeSegments(segments,voiceId,onProgress) Data
}
class ServerAPIClient {
+requestTTS(text,voiceId,rate) Data
+requestVoiceClone(audioData,voiceName) String
}
CosyVoiceService --> ServerAPIClient : "调用"
```

图表来源
- [CosyVoiceService.swift:1-104](file://Services/CosyVoiceService.swift#L1-L104)
- [ServerAPIClient.swift:47-97](file://Services/ServerAPIClient.swift#L47-L97)

章节来源
- [CosyVoiceService.swift:1-104](file://Services/CosyVoiceService.swift#L1-L104)

### Edge TTS 云端语音合成服务（EdgeTTSService）
职责
- 通过 naolizhi.cn 中转调用微软 Edge TTS
- 提供免费无限量的高质量中文语音合成
- 支持丰富的中文音色选择（晓晓、云希、云扬等）
- 自动处理语速转换与请求优化

**新增** 这是新添加的免费 TTS 服务，音质接近 Azure Neural TTS

```mermaid
classDiagram
class EdgeTTSService {
+synthesize(text,voice,rate) Data
+convertRate(rate) String
+allVoices : [EdgeVoice]
}
class EdgeVoice {
+id : String
+name : String
+gender : String
+tag : String?
+recommendedChinese : [EdgeVoice]
+recommendedCantonese : [EdgeVoice]
}
EdgeTTSService --> EdgeVoice : "使用"
```

图表来源
- [EdgeTTSService.swift:1-133](file://Services/EdgeTTSService.swift#L1-L133)

章节来源
- [EdgeTTSService.swift:1-133](file://Services/EdgeTTSService.swift#L1-L133)

### Edge TTS 引擎适配器（EdgeTTSSynthesizer）
职责
- 实现 SpeechSynthesizerProtocol 协议
- 通过 EdgeTTSService 进行云端语音合成
- 支持段落预加载与无缝衔接播放
- 提供完整的播放控制功能

**新增** 这是新添加的 TTS 引擎适配器，支持高级播放功能

```mermaid
stateDiagram-v2
[*] --> Idle
Idle --> Playing : "speak()"
Playing --> Paused : "pause()"
Paused --> Playing : "resume()"
Playing --> Finished : "完成/下一段"
Finished --> Idle : "stop/reset"
```

图表来源
- [EdgeTTSSynthesizer.swift:1-275](file://Services/EdgeTTSSynthesizer.swift#L1-L275)

章节来源
- [EdgeTTSSynthesizer.swift:1-275](file://Services/EdgeTTSSynthesizer.swift#L1-L275)

### 本地播放服务（SpeechService）
职责
- 基于系统 TTS 的分段朗读、暂停/继续、跳转
- 维护播放状态与位置回调
- 适配不同引擎与语言配置

```mermaid
stateDiagram-v2
[*] --> Idle
Idle --> Playing : "speak()"
Playing --> Paused : "pause()"
Paused --> Playing : "resume()"
Playing --> Finished : "完成/下一段"
Finished --> Idle : "stop/reset"
```

图表来源
- [SpeechService.swift:1-166](file://Services/SpeechService.swift#L1-L166)
- [PlaybackState.swift:1-9](file://Models/PlaybackState.swift#L1-L9)

章节来源
- [SpeechService.swift:1-166](file://Services/SpeechService.swift#L1-L166)
- [PlaybackState.swift:1-9](file://Models/PlaybackState.swift#L1-L9)

### 音频会话服务（AudioSessionService）
职责
- 统一配置与激活/停用 AVAudioSession
- 支持后台播放、蓝牙与 AirPlay

章节来源
- [AudioSessionService.swift:1-46](file://Services/AudioSessionService.swift#L1-L46)
- [AppDelegate.swift:1-14](file://App/AppDelegate.swift#L1-L14)
- [Info.plist:1-44](file://Resources/Info.plist#L1-L44)

### 视图与集成（ContentView）
职责
- 作为主入口承载多个 Tab
- 协调分享导入、错误提示与服务调用

章节来源
- [ContentView.swift:1-98](file://Views/ContentView.swift#L1-L98)

## 依赖关系分析
- 低耦合：各业务服务仅依赖统一的 ServerAPIClient，避免重复网络实现
- 高内聚：每个服务聚焦单一职责（摘要/伴读/TTS/播放）
- 外部依赖：AVFoundation（TTS/音频）、SwiftData（持久化，由 App 注入）
- 潜在循环：未发现循环依赖；服务间通过单例共享客户端

```mermaid
graph LR
AISummaryService --> ServerAPIClient
CompanionService --> ServerAPIClient
CosyVoiceService --> ServerAPIClient
EdgeTTSService --> EdgeTTSSynthesizer
SpeechService --> AudioSessionService
ContentView --> AISummaryService
ContentView --> CompanionService
ContentView --> CosyVoiceService
ContentView --> EdgeTTSSynthesizer
```

图表来源
- [AISummaryService.swift:1-90](file://Services/AISummaryService.swift#L1-L90)
- [CompanionService.swift:1-47](file://Services/CompanionService.swift#L1-L47)
- [CosyVoiceService.swift:1-104](file://Services/CosyVoiceService.swift#L1-L104)
- [EdgeTTSService.swift:1-133](file://Services/EdgeTTSService.swift#L1-L133)
- [EdgeTTSSynthesizer.swift:1-275](file://Services/EdgeTTSSynthesizer.swift#L1-L275)
- [SpeechService.swift:1-166](file://Services/SpeechService.swift#L1-L166)
- [AudioSessionService.swift:1-46](file://Services/AudioSessionService.swift#L1-L46)
- [ContentView.swift:1-98](file://Views/ContentView.swift#L1-L98)

章节来源
- [ServerAPIClient.swift:1-206](file://Services/ServerAPIClient.swift#L1-L206)

## 性能与可靠性
- 超时策略：默认请求与资源超时分别设置为 60s/120s，TTS 单独设为 30s，兼顾长文本与音频传输
- 重试建议：对可重试的网络错误（如 429/5xx）可在上层增加指数退避重试
- 分片与节流：长文本 TTS 已内置分段与 200ms 间隔，降低瞬时压力
- 内存与带宽：大音频尽量流式处理或按需下载，避免一次性加载
- 降级路径：当云端不可用时，可回退到系统 TTS（VoiceConfig.engine 切换）
- **新增** Edge TTS 预加载机制：后台预加载下一段音频，实现无缝衔接播放

[本节为通用指导，不直接分析具体文件]

## 故障排查指南
常见问题与定位思路
- 未授权/配额限制：检查服务端鉴权与配额策略，确认客户端是否正确传递必要字段
- 响应格式异常：核对服务端返回是否包含 result 或 content 字段；必要时在服务端统一包装
- 无音频数据：确认 Content-Type 或 JSON 中 audio_url/audio 字段是否存在且可达
- 网络错误：查看底层错误描述，结合日志级别输出定位
- **新增** Edge TTS 问题：检查 naolizhi.cn 服务可用性，确认音色 ID 格式正确

错误映射与日志
- 服务端错误统一映射为结构化错误类型，便于 UI 展示与统计
- 全局错误处理器提供弹窗与日志输出，便于快速复现问题

章节来源
- [ServerAPIClient.swift:161-206](file://Services/ServerAPIClient.swift#L161-L206)

## 结论
本项目以"服务器代理"为核心，将 AI 能力与安全边界收敛在服务端，客户端通过轻量化的代理客户端与清晰的服务分层，实现了可扩展、易维护的架构。未来可在以下方向持续演进：
- 引入统一认证与签名机制，增强接口安全性
- 完善错误上报与指标采集，提升可观测性
- 优化长文本与音频的流式传输体验
- 增加缓存与离线策略，提高鲁棒性与用户体验
- **新增** 考虑集成更多免费 TTS 服务，提供更多音色选择

[本节为总结性内容，不直接分析具体文件]

## 附录
- 配置项说明
  - baseURL：中转服务器地址，现为 `https://naolizhi.cn/api`
  - 超时：请求/资源/特定接口的超时时间
  - 引擎选择：VoiceConfig.engine 决定使用系统 TTS 或云端 TTS
- 部署清单
  - 后端需提供 /summary、/companion、/tts、/voice-clone 四个接口
  - 确保返回 JSON 包含 result 或 content 字段，或兼容 output.choices.message.content
  - TTS 接口返回音频二进制或包含 audio_url/audio base64 的 JSON
- **新增** Edge TTS 配置
  - 端点：`https://naolizhi.cn/api/tts`
  - 支持丰富的中文音色，无需额外注册
  - 免费无限量使用，音质接近 Azure Neural TTS

章节来源
- [ServerAPIClient.swift:11-23](file://Services/ServerAPIClient.swift#L11-L23)
- [EdgeTTSService.swift:10-11](file://Services/EdgeTTSService.swift#L10-L11)
- [VoiceConfig.swift:1-71](file://Models/VoiceConfig.swift#L1-L71)