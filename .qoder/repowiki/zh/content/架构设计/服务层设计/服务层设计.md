...
## 更新摘要
**变更内容**   
- **重大架构升级**：Edge TTS 实现引入背景预加载系统，显著提升播放流畅性
- **任务管理优化**：采用 Task 取消和 continuation-based 同步机制替代委托回调方式
- **内存管理增强**：在 stop() 方法中完善资源清理，包括预加载任务和音频播放器释放
- **无缝衔接播放**：实现段落间零延迟切换，通过后台预加载下一段音频数据
- **错误处理改进**：更完善的任务取消检查和状态同步机制
...
## 引言
本设计文档聚焦 Knowledge 应用的服务层，围绕 SpeechSynthesizerProtocol 抽象接口的设计理念与扩展机制展开，说明如何通过协议抽象统一系统 TTS 与 AI 语音引擎；并阐述各服务的职责、依赖与通信方式，包括错误处理策略与服务降级机制。**最新更新**：Edge TTS 引擎实现了重大架构升级，引入背景预加载系统和改进的任务管理机制，提供无缝衔接的云端语音合成体验。同时增强了触觉反馈服务和 AI 伴读功能，为播放控制、进度条拖拽、语速切换等关键交互提供一致的触觉反馈体验，并支持边听边问的交互式体验。最后提供面向新语音引擎或 AI 服务的扩展开发指南，帮助开发者快速接入新的能力。
...
- **EdgeTTSSynthesizer**：**重大升级** Edge TTS 云端语音合成引擎适配器，通过自建服务器中转调用微软 Edge TTS，提供免费的 Neural TTS 服务，现已实现背景预加载和无缝衔接播放。
...
## 架构总览
服务层采用"协议抽象 + 具体实现"的分层模式。上层通过 SpeechSynthesizerProtocol 与任意语音引擎交互，SpeakerViewModel 作为门面负责组合多个服务（音频会话、远程控制、错误处理、AI 摘要、AI 伴读、触觉反馈）并提供统一的业务接口。**最新重大升级**：Edge TTS 引擎实现了革命性的架构改进，通过背景预加载系统和改进的任务管理机制，提供无缝衔接的云端语音合成体验。
...
### Edge TTS 云端语音引擎（重大架构升级）
- **EdgeTTSSynthesizer 重大升级**
  - **背景预加载系统**：实现智能预加载机制，在播放当前段落的同时后台合成下一段音频，消除段落间延迟。
  - **改进的任务管理**：采用 Task 取消和 continuation-based 同步机制替代传统的委托回调方式，提供更可靠的异步控制。
  - **增强的内存管理**：在 stop() 方法中完善资源清理，包括预加载任务、音频播放器、continuation 对象和缓存数据的释放。
  - **无缝衔接播放**：通过 prefetchedData 缓存和 playbackContinuation 同步机制，实现段落间的零延迟切换。
- **技术实现细节**
  - **预加载缓存**：`prefetchedData: (index: Int, data: Data)?` 存储下一段的音频数据
  - **任务同步**：`playbackContinuation: CheckedContinuation<Void, Never>?` 实现 Task 等待播放结束
  - **后台预加载**：独立的 `prefetchTask` 在后台运行，不阻塞主播放循环
  - **智能取消**：在播放新段落前自动取消上一次预加载任务，避免资源浪费
- **EdgeTTSService 功能**
...
Service->>Server : POST /api/tts
Server-->>Service : MP3音频数据
Service-->>Syn : audioData
end
Syn->>Player : 写入临时文件并播放
Syn->>Syn : 启动后台预加载下一段
Service->>Server : 预加载下一段
Server-->>Service : 预加载音频数据
Service-->>Syn : 存入prefetchedData缓存
Player-->>Syn : 播放完成回调
Syn->>Syn : continuation.resume()
Syn-->>VM : onPositionChange / onRangeChange
end
Syn-->>VM : state=finished
```
...
**图表来源**
- [EdgeTTSSynthesizer.swift:149-223](file://Services/EdgeTTSSynthesizer.swift#L149-L223)
- [EdgeTTSService.swift:63-91](file://Services/EdgeTTSService.swift#L63-L91)
- [SpeakerViewModel.swift:215-266](file://ViewModels/SpeakerViewModel.swift#L215-L266)
...
- **低耦合**
  ...
  - **重大升级**：Edge TTS 服务完全解耦于其他引擎，遵循相同的协议规范，但引入了更复杂的预加载机制。
...
- **单一职责**
  ...
  - **重大升级**：EdgeTTSService 专注 Edge TTS 网络请求；EdgeTTSSynthesizer 专注云端语音播放编排与预加载管理。
...
- **性能特性**
  ...
  - **重大升级**：Edge TTS：通过自建服务器中转，提供稳定的云端服务；**新增背景预加载系统**，在播放当前段落的同时后台合成下一段，实现无缝衔接播放；**改进的任务管理机制**，通过 Task 取消和 continuation 同步提供更可靠的异步控制。
...
- **优化建议**
  ...
  - 合并小段音频以减少 I/O 次数。
  - 引入缓存策略，复用已合成的音频片段。
  - **重大升级**：Edge TTS 预加载系统已实现智能缓存，可根据播放进度动态调整预加载时机。
...
- **扩展性**
  ...
  - **重大升级**：参考 EdgeTTSSynthesizer 的背景预加载和任务管理机制，为新引擎提供高性能的云端合成能力。
...
- **常见问题**
  ...
  - **重大升级**：Edge TTS 预加载异常：检查 prefetchTask 是否正确取消，验证 prefetchedData 缓存状态，确认 continuation 同步机制正常工作。
  - 播放不同步：确认 onPositionChange/onRangeChange 是否在主线程回调；检查计时器与播放器委托。
  - 音频无法播放：验证临时文件路径与权限；检查 AVAudioPlayer 初始化异常。
  - **重大升级**：Edge TTS 任务泄漏：检查 stop() 方法是否正确清理 synthesisTask、prefetchTask、audioPlayer 和 playbackContinuation。
...
- **错误处理策略**
  ...
  - 引擎级错误向上抛出，上层进行降级或提示。
  - **重大升级**：Edge TTS 错误分类处理，区分网络错误、API 错误、响应解析错误，并包含任务取消状态的检查。
...
- **调试技巧**
  ...
  - **重大升级**：打印 Edge TTS 预加载状态、任务取消信息和 continuation 同步状态，验证预加载系统正确性。
...
- [EdgeTTSService.swift:111-132](file://Services/EdgeTTSService.swift#L111-L132)
...
- [SpeakerViewModel.swift:322-335](file://ViewModels/SpeakerViewModel.swift#L322-L335)
- [EdgeTTSSynthesizer.swift:69-79](file://Services/EdgeTTSSynthesizer.swift#L69-L79)

## 结论
服务层通过清晰的协议抽象与职责分离，实现了系统 TTS、Edge TTS 与 AI 语音的统一接入与无缝切换。**重大升级**：Edge TTS 引擎实现了革命性的架构改进，通过背景预加载系统和改进的任务管理机制，消除了云端语音合成的延迟问题，提供了接近本地 TTS 的流畅体验。新增的 HapticService 触觉反馈服务提升了用户体验，为播放控制、进度条拖拽、语速切换等关键交互提供了即时的触觉反馈。CompanionService AI 伴读服务丰富了服务层的 AI 能力，支持边听边问的交互式体验。SpeakerViewModel 作为门面编排播放、AI 摘要、AI 伴读、触觉反馈与系统媒体集成，配合 ErrorHandler 提供一致的错误处理体验。该设计具备良好的可扩展性与可测试性，便于后续接入更多语音引擎与 AI 能力。
...
- **注意事项**
  ...
  - **重大升级**：参考 EdgeTTSSynthesizer 的背景预加载和任务管理机制，实现高性能的云端合成。
...
- **步骤**
  ...
  4. **重大升级**：实现背景预加载系统，包括预加载缓存、后台任务和 continuation 同步机制。
  5. 在 TTSEngine 枚举中添加新的引擎类型。
  6. 在 SpeakerViewModel 中集成新引擎的切换逻辑。
  7. 创建音色选择界面，提供用户友好的音色配置体验。
- **最佳实践**
  - 实现合理的文本分段策略，避免单次请求过大。
  - 提供详细的错误分类和用户友好的错误提示。
  - 支持音色预览功能，让用户在选择前试听效果。
  - 考虑网络异常情况下的降级策略。
  - **重大升级**：实现智能预加载机制，在播放当前段落的同时后台合成下一段，消除段落间延迟。
  - **重大升级**：使用 Task 取消和 continuation-based 同步机制替代传统委托回调，提供更可靠的异步控制。
  - **重大升级**：在 stop() 方法中完善资源清理，包括所有后台任务、缓存数据和播放器资源的释放。
