...
## 更新摘要
**变更内容**   
- 新增 HapticService 触觉反馈管理服务，提供统一的触觉交互体验
- 在播放控制、进度条拖拽、语速切换等关键交互点集成触觉反馈
- 预初始化多种 UIImpactFeedbackGenerator 实例，优化触觉响应性能
- 为不同交互类型提供专用方法：播放/暂停、快进/快退、段落边界跨越、语速变化等
...
## 引言
本设计文档聚焦 Knowledge 应用的服务层，围绕 SpeechSynthesizerProtocol 抽象接口的设计理念与扩展机制展开，说明如何通过协议抽象统一系统 TTS 与 AI 语音引擎；并阐述各服务的职责、依赖与通信方式，包括错误处理策略与服务降级机制。**最新更新**：新增了 HapticService 触觉反馈管理服务，为播放控制、进度条拖拽、语速切换等关键交互提供一致的触觉反馈体验，增强了用户交互的沉浸感。最后提供面向新语音引擎或 AI 服务的扩展开发指南，帮助开发者快速接入新的能力。
...
## 架构总览
服务层采用"协议抽象 + 具体实现"的分层模式。上层通过 SpeechSynthesizerProtocol 与任意语音引擎交互，SpeakerViewModel 作为门面负责组合多个服务（音频会话、远程控制、错误处理、AI 摘要、AI 伴读、触觉反馈）并提供统一的业务接口。
...
## 结论
服务层通过清晰的协议抽象与职责分离，实现了系统 TTS 与 AI 语音的统一接入与无缝切换。**最新增强**：新增的 HapticService 触觉反馈服务进一步提升了用户体验，为播放控制、进度条拖拽、语速切换等关键交互提供了即时的触觉反馈。同时，CompanionService AI 伴读服务丰富了服务层的 AI 能力，支持边听边问的交互式体验。SpeakerViewModel 作为门面编排播放、AI 摘要、AI 伴读、触觉反馈与系统媒体集成，配合 ErrorHandler 提供一致的错误处理体验。该设计具备良好的可扩展性与可测试性，便于后续接入更多语音引擎与 AI 能力。
