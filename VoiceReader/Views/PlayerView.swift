                ...
                if let doc = speakerVM.currentDocument {
                    VStack(spacing: 24) {
                        ...
                        ZStack {
                            ...
                            VStack(spacing: 8) {
                                Image(systemName: doc.fileType.iconName).font(.system(size: 40)).foregroundColor(.white)
                                Text(doc.fileType.displayName).font(.caption).fontWeight(.bold).foregroundColor(.white.opacity(0.8))
                            }
                        }
                        // 信息
                        VStack(spacing: 6) {
                            Text(doc.title).font(.title2).fontWeight(.bold).lineLimit(2).multilineTextAlignment(.center)
                            Text("\(doc.fileType.displayName) · \(formatLen(doc.totalLength))").font(.subheadline).foregroundColor(.secondary)
